...
resource "aws_db_instance" "postgres" {
  ...
  name              = "debtcollective_${var.environment}"
...
data "template_file" "knexfile" {
  ...
  vars {
    database_url = "postgres://${var.db_username}:${var.db_password}@${aws_db_instance.postgres.address}:${aws_db_instance.postgres.port}/debtcollective_${var.environment}"
  }
}
...
resource "aws_route53_record" "www" {
  ...
  name    = "staging.debtcollective.org"
  type    = "A"
